// This a minimal fully functional example for setting up a server written in
// C++ that communicates with clients via WebRTC data channels. This uses
// WebSockets to perform the WebRTC handshake (offer/accept SDP) with the
// client. We only use WebSockets for the initial handshake because TCP often
...
void OnIceCandidate(const webrtc::IceCandidateInterface* candidate) {
    ...
    message_object.AddMember("type", "candidate",
    ...
    message_payload.AddMember("candidate", candidate_value,
                              message_object.GetAllocator());
    message_payload.AddMember("sdpMid", sdp_mid_value,
                              message_object.GetAllocator());
...
void OnWebSocketMessage(WebSocketServer* /* s */,
    ...
    if (type == "ping") {
        ...
        int sdp_mline_index =
                message_object["payload"]["sdpMLineIndex"].GetInt();
...
int main() {
    ...
    // In a real game server, you would run the WebSocket server as a separate
    // thread so your main process can handle the game loop.
    ws_server.set_message_handler(
            bind(OnWebSocketMessage, &ws_server, ::_1, ::_2));
    ws_server.init_asio();
